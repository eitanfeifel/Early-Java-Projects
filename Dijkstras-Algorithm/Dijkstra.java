...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // TODO
        return ( double )( Math.sqrt( ( ( vy - uy ) * ( vy - uy ) ) + ( ( vx - ux ) * ( vx - ux ) ) ) ); 
  }
  ...
  public void computeAllEuclideanDistances() {
        // TODO
        for( String w : vertexNames.keySet( ) ) {

            for( Edge e : vertexNames.get( w ).adjacentEdges ) {

                e.distance = computeEuclideanDistance( vertexNames.get( w ).x, vertexNames.get( w ).y,
  ...
  public void doDijkstra(String s) {
    ...
    for ( String v : vertexNames.keySet( ) )
    {
      vertexNames.get( v ).prev = null;
      vertexNames.get( v ).distance = Double.MAX_VALUE;
      vertexNames.get( v ).known = false;
      Vertices.add( v ); 
    }
      ...
      while ( !Vertices.isEmpty() )
      {
        ...
        {
          if ( dist > vertexNames.get(u).distance)
          {
            dist = vertexNames.get(u).distance;
            z = u;
          }
        }
        Vertices.remove(z);
        vertexNames.get(z).known = true;

      for( Edge e : vertexNames.get(z).adjacentEdges)
      {
        if( e.target.known == false )
        {
          double newD = vertexNames.get(z).distance + e.distance;
          if ( e.target.distance > newD )
          {
            e.target.distance = newD;
            e.target.prev = vertexNames.get(z);
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    // TODO
    List<Edge> dPath = new ArrayList<>(); 
    Vertex destination = vertexNames.get( t );
    Vertex from = destination.prev;
    while(from != null)
    {
      for ( Edge e: from.adjacentEdges )
      {
        if ( e.target.name.equals( destination.name ) )
        {
          dPath.add(e);
          break;
        }
